...
% Declared here so Overleaf identifies main.tex as the entry point.
...
% 博士班就把下方註解拿掉吧!! (碩士論文請保持註解)
%\toggletrue{iamphd}

% 論文名稱
\newcommand{\chineseTitle}{針對 RF 自動調變分類中對抗式攻擊的即時頻域防禦方法}
\newcommand{\englishTitle}{Real-Time Defense Against Adversarial Attacks in RF Automatic Modulation Classification}
% 論文名稱太長的話, 可以修改covers/inside_var.tex 的顯示語法, 詳情寫在那邊

% 自己的名字 (英文名字有兩種寫法, 一個是姓在後, 一個是放前面還有逗號. )
\newcommand{\studentCnName}{王彥凱}
\newcommand{\studentEnName}{wang, yan kai} % 書名頁
\newcommand{\studentEnNameA}{wang, yan kai} % 封面用

% 教授的名字
\newcommand{\advisorCnName}{葉宗泰}
\newcommand{\advisorEnName}{yeh, tsung tai}     % 書名頁
\newcommand{\advisorEnNamex}{yeh, tsung tai}   % 封面用
...
\newcommand{\ThesisDateTW}{中華民國~ 一一五年七月}
