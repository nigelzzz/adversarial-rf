...
def Load_Dataset(dataset,
    ...
    if dataset == '2016.10a' or dataset == '2016.10b':
        ...
        all_snrs, all_mods = map(lambda j: sorted(list(set(map(lambda x: x[j], Set.keys())))), [1, 0])
        ...
        if mod_filter is not None:
            ...
            sel_mods = [m for m in all_mods if m == mod_filter]
        sel_snrs = all_snrs
        ...
        for mod in sel_mods:
            for snr in sel_snrs:
                arr = Set[(mod, snr)]
                Signals.append(arr)
                Labels.extend([mod] * arr.shape[0])
                SNRs.extend([snr] * arr.shape[0])
