...
We evaluate eight defense configurations: the proposed Adaptive-K v2 and
seven baselines (Kalman, Wiener, Savitzky-Golay, Gaussian, FIR, and
Randomized Smoothing), plus the undefended baseline.  The proposed method
operates entirely in the frequency domain without retraining and requires
no per-SNR calibration.
...
\caption{Defense configurations. Classical filter parameters are calibrated
  ...
  SNR $= 10$\,dB are shown. Adaptive-K v2 requires no per-SNR calibration.}
\label{tab:defenses}
\centering
\small
...
\midrule
Adaptive-K v2   & Proposed & Per-sample $K$ via magnitude knee; SNR-adaptive cap $K_{\max}\in[20,35]$; wideband routing at flatness $>0.4$ \\
